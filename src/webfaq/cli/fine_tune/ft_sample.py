...
def ft_sample(dataset_name: str, repo_id: str):
    ...
    for language, limit in LANGUAGES.items():
        ...
        list_qrels = []
        for d in tqdm(
            load_dataset(repo_id, f"{language}-qrels")["train"], desc="Mapping"
        ):
            list_qrels.append((d["query-id"], d["corpus-id"]))

        dict_queries = {}
        for d in tqdm(
            load_dataset(repo_id, f"{language}-queries")["queries"], desc="Mapping"
        ):
            dict_queries[d["_id"]] = d["text"]

        dict_corpus = {}
        for d in tqdm(
            load_dataset(repo_id, f"{language}-corpus")["corpus"], desc="Mapping"
        ):
        ...
        if i + limit > len(list_qrels):
            list_qrels_train = (
                list_qrels[i:] + list_qrels[: i + limit - len(list_qrels)]
            )
        else:
        ...
        train_path = os.path.join(
            DATASETS_FOLDER, dataset_name, "fine_tune", language, "train.jsonl"
        )
