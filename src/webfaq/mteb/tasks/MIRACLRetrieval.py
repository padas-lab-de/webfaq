...
def _load_miracl_data(
    path: str, lang: dict, splits: str, cache_dir: str = None, revision: str = None
):
    corpus = {split: None for split in splits}
    queries = {split: None for split in splits}
    relevant_docs = {split: None for split in splits}
    ...
    for row in corpus_data["corpus"]:
        ...
        doc_title = row["title"]
        doc_text = row["text"]
        corpus[split][docid] = {"title": doc_title, "text": doc_text}
